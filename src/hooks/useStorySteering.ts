...
export const useStorySteering = () => {
  ...
  const handleAlterFate = async (chapterNumber: number, direction: string, customPrompt: string) => {
    ...
    const activeStory = currentStoreState.stories.find(s => s.id === currentStoreState.activeStoryId);
    if (!activeStory) return;
    const alterFateLockMessage = getFateLockMessage(activeStory, chapterNumber);
    if (alterFateLockMessage) {
      currentStoreState.setAppError(alterFateLockMessage);
      return;
    }
