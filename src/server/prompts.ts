...
Additionally, emit a per-scene 'music' object inside "metadata" when the scene's backing soundtrack can be described or changes: { "mood": "war"|"duel"|"serenity"|"romance"|"dread"|"mystery"|"triumph"|"tribulation"|"travel"|"tragedy"|"fighting"|"adventure"|"ambient"|"boss-fight"|"tension"|"sad"|"mystical"|"excitement"|"tired"|"horror", "region": "chinese"|"japanese"|"western" (optional), "intensity": number (optional, 0 to 1) }. For looping atmospheric audio, include concrete "environment" terms and optional "atmosphereTags" only when they are directly established by the scene (for example "coast", "harbor", "rain", "market", "battle", "machinery"). These are catalog-match terms, not sound names: never infer rain merely because characters are travelling, and do not invent a generic atmosphere when the setting is unclear.
...
NARRATIVE SURFACE HYGIENE:
Story tags, trope labels, style notes, app/director instructions, casual user shorthand, Fate Pressure, Fate Survival, death flags, hidden timers, fate locks, doom deadlines, fate scars, destiny shifts, and timeline changes are control signals only. They may influence character behavior, scene structure, pacing, stakes, consequences, emotional direction, and structured system blocks, but must not appear verbatim in normal narration or dialogue unless natural in-world language for this story.
Translate control language into story action: show the emotional behavior rather than naming "enemies to lovers" tension; render "mid" as a character being unimpressed by stale orthodoxy or hollow tradition unless the story has an explicit modern/comedic voice. Do not narrate Fate mechanics such as "this action shifted destiny," "the timeline locked," or a hidden timer unless the story explicitly has an in-world System/LitRPG interface. A moment that needs visible UI treatment (Fate events included, in ANY genre) may be rendered as a standalone system line on its own paragraph (see CELESTIAL LIBRARY SYSTEM PANELS below); never embed bracketed alerts or control labels inside narration or dialogue sentences. In non-System/LitRPG stories the surrounding narration stays natural — prose carries consequences, omens, pressure, and character choices, while the system line appears selectively at the moments that earn it.

CELESTIAL LIBRARY SYSTEM PANELS (ALL GENRES):
System panels are a signature Celestial Library storytelling device available in EVERY genre, not just System/LitRPG stories. In this legacy JSON response, a visible panel is written as a single standalone line in "chapterText", separated by blank lines from surrounding prose, wrapped entirely in square brackets with an in-world title (e.g. "[Technique Learned: Azure Sky Sword Art — Initial Mastery]"). Emit one when a moment genuinely benefits from visible UI treatment: learning a technique or skill, discovering an ability or artifact, a breakthrough or meaningful progression, important Codex information, karma/relationship/choice consequences, warnings, revelations, prophecies, mysteries, major Fate events, or significant world-state changes.
Genre-aware intensity:
- Explicit System/LitRPG stories: panels may be frequent and openly mechanical (stats, levels, quests).
- Cultivation, fantasy, progression, academy, crafting, beast-taming, and similar genres: selective panels for important progression, knowledge, techniques, artifacts, or consequences, styled to the world's own voice.
- Romance, mystery, cozy, political, and grounded stories: rare panels reserved for meaningful bonds, discoveries, warnings, records, or turning points.
Do not force a panel into every chapter, and never manufacture an empty event just to display one. Panels are never restricted to Fate events alone. Never place raw control labels or editor instructions inside these lines or anywhere in prose.
...
3. Describe the opening scene with "sceneType", "environment" (array), and optional "atmosphereTags" using only concrete, audible setting terms. These select a curated looping atmosphere by catalog tags; they must never name an asset or assume rain because the chapter involves travel. Omit uncertain terms rather than guessing.
