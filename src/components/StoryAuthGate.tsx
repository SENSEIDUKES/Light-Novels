...
/**
 * Time (ms) the gate lingers after sign-in so the shell can dissolve while the
 * world is still visible. CreationModal waits this long before resuming the
 * pending generation action.
 */
...
export default function StoryAuthGate() {
  ...
  // The gate stays mounted briefly after sign-in (CreationModal keeps the
  // pending action alive for STORY_AUTH_DISSOLVE_MS) so the shell can dissolve
  // over the still-visible world instead of vanishing instantly.
  useEffect(() => {
  ...
  const handleGoogleSignIn = () => {
    void runSignIn('google', () => signInWithPopup(auth, new GoogleAuthProvider()));
  };

  const handleAppleSignIn = () => {
    void runSignIn('apple', () => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 flex min-h-[100dvh] flex-col items-center justify-center pt-safe pb-safe [padding-left:max(1.25rem,env(safe-area-inset-left))] [padding-right:max(1.25rem,env(safe-area-inset-right))]">
        <motion.div
        ...
        >
          <div className="flex flex-col items-center text-center gap-5">
            ...
            <div className="w-full space-y-3">
              ...
              >
                {pendingProvider === 'email' ? <Spinner /> : <Mail size={18} className="gold-accent" />}
