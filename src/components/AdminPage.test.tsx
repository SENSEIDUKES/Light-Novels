...
describe('AdminPage', () => {
  it('renders without crashing', () => {
    const { container } = render(<AdminPage memory={createMemory()} onUpdateMemory={() => {}} />);
  ...
  it('toggles only the selected character when names are duplicated', () => {
    ...
    render(<AdminPage memory={memory} onUpdateMemory={(next) => { updatedMemory = next; }} />);
  ...
  it('keeps in-progress character drafts when switching tabs', () => {
    render(<AdminPage memory={createMemory()} onUpdateMemory={() => {}} />);
