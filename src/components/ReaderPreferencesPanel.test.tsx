...
describe('ReaderPreferencesPanel', () => {
  ...
  it('allows customizing the particle motif style and triggers updates', async () => {
    const handleUpdatePreference = vi.fn();
    render(
      <ReaderPreferencesPanel
    ...
    const motifToggle = screen.getByRole('button', { name: /Particle Motif/i });
    expect(motifToggle.getAttribute('aria-expanded')).toBe('true');
