...
describe('ReaderPreferencesPanel', () => {
  ...
  it('allows customizing the particle style and triggers updates', async () => {
    ...
    const particleStyleToggle = screen.getByRole('button', { name: /Atmospheric Style/i });
    expect(particleStyleToggle.getAttribute('aria-expanded')).toBe('true');
    expect(screen.getByRole('button', { name: /Sword Qi Sparks/i })).toBeDefined();

    fireEvent.click(screen.getByRole('button', { name: /Sword Qi Sparks/i }));
