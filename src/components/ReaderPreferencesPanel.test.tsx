...
describe('ReaderPreferencesPanel', () => {
  ...
  it('allows customizing the particle motif and triggers updates', async () => {
    const handleUpdatePreference = vi.fn();
    render(
      <ReaderPreferencesPanel
    ...
    const particleMotifToggle = screen.getByRole('button', { name: /Particle Motif/i });
    expect(particleMotifToggle.getAttribute('aria-expanded')).toBe('true');
