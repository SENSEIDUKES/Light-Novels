...
import { Camera, Sparkles, BookOpen, Clock, ArrowRight, User, Download, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { StoryWorld, StoryMemory } from '../../types';
...
const CollageItem = React.memo(({ mem, isDownloading, handleDownload, handleOpenLightbox }: CollageItemProps) => {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, scale: 1, y: 0, rotate: mem.tiltAngle }}
      exit={{ opacity: 0, scale: 0.9, y: 10 }}
      whileHover={{ scale: 1.05, rotate: 0, zIndex: 30, boxShadow: '0 10px 30px rgba(4,172,255,0.25)' }}
...
export function LivingCodexCollage({
  ...
  const safeFormatDate = useCallback((dateVal: unknown) => {
    ...
    activeStory.arcs?.forEach((arc) => {
      arc.chapters?.forEach((ch) => {
        if (ch.assetManifest?.heroImage) {
          ...
          seenUrls.add(url);
          const tiltAngle = ((ch.number * 7) % 10) - 5;
  ...
  const filteredMemories = useMemo(() => {
    if (filter === 'scenes') return memories.filter((m) => m.type === 'scene');
    if (filter === 'entities') return memories.filter((m) => m.type !== 'scene');
    return memories;
  }, [memories, filter]);
                ...
                <div className="pt-4 border-t border-neutral-900 flex flex-col gap-2">
                  {selectedMemory.chapterNumber && onJumpToChapter && (
                    <button
                      ...
                      tabIndex={0}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }}
                      onClick={() => handleRevisitScene(selectedMemory.chapterNumber!)}
