...
import { PlotThread, StoryMemory } from '../../types';
...
const getThreadKey = (thread: string | PlotThread) => {
  if (typeof thread === 'string') return thread;
  if (thread.id) return thread.id;
  return JSON.stringify({
    description: thread.description,
    originChapter: thread.originChapter,
    sourceBlockId: thread.provenance?.sourceBlockId
  });
};

const getThreadText = (thread: string | PlotThread) => (
  typeof thread === 'object' && 'description' in thread ? thread.description : String(thread)
);
        ...
        <div className="space-y-1.5">
          {memory.unresolvedPlotThreads.length === 0 ? (
            ...
            memory.unresolvedPlotThreads.map((thread, idx) => (
              <div key={getThreadKey(thread)} className="flex items-center justify-between gap-2 p-2.5 bg-void/50 border border-neutral-900 rounded text-xs text-neutral-300">
                <div className="flex items-center min-w-0 pr-1">
                  <Compass className="text-neutral-600 flex-shrink-0 mr-2" size={13} />
                  <span className="truncate" title={typeof thread === 'object' ? JSON.stringify(thread.provenance, null, 2) : ''}>{getThreadText(thread)}</span>
                </div>
                <button
                  type="button"
        ...
        <div className="space-y-1.5">
          {(memory.resolvedPlotThreads || []).length === 0 ? (
            ...
            (memory.resolvedPlotThreads || []).map((thread, idx) => (
              <div key={getThreadKey(thread)} className="flex items-center justify-between gap-2 p-2 bg-neutral-950 border border-neutral-950 text-neutral-500 rounded text-xs group">
                <div className="flex items-center min-w-0 pr-1">
                  <CheckCircle2 className="text-green-800 flex-shrink-0 mr-2" size={13} />
                  <span className="line-through truncate italic" title={typeof thread === 'object' ? JSON.stringify(thread.provenance, null, 2) : ''}>{getThreadText(thread)}</span>
                </div>
