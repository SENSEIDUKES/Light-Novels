...
export const ParticleSystem: React.FC<ParticleSystemProps> = React.memo(({ 
  ...
  const particles = useMemo(() => {
    return Array.from({ length: count }).map((_, i) => {
      ...
      const rotationSpeed = (Math.random() - 0.5) * 0.5; // rads per second
  ...
  useEffect(() => {
    ...
    // Calculate required canvas size based on particle style
    let maxDrawSize = baseSize;
    if (particleStyle === 'sword_qi') {
      maxDrawSize = baseSize * 2.5; // Long shards
    } else if (particleStyle === 'lotus_blossom') {
      maxDrawSize = baseSize * 1.5; // Petals
    }

    const blur = 8;
    const padding = blur * 2;
    const canvasSize = maxDrawSize + padding * 2;
    ...
    if (offCtx) {
      ...
      if (particleStyle === 'sword_qi') {
         // Sharp angled shard
         const length = baseSize * 2.5;
         const width = baseSize * 0.5;
         offCtx.moveTo(-length/2, 0);
         offCtx.lineTo(-length/4, -width/2);
         offCtx.lineTo(length/2, 0);
         offCtx.lineTo(-length/4, width/2);
         offCtx.closePath();
      } else if (particleStyle === 'lotus_blossom') {
         // Petal shape (ellipse)
         const length = baseSize * 1.5;
         const width = baseSize * 0.8;
         offCtx.ellipse(0, 0, length/2, width/2, 0, 0, Math.PI * 2);
      } else {
         // Default circle
    ...
    const render = (time: number) => {
      ...
      particles.forEach(p => {
        ...
        if (particleStyle === 'sword_qi' || particleStyle === 'lotus_blossom') {
          const currentRotation = p.rotation + (p.rotationSpeed * (elapsed / 1000));
