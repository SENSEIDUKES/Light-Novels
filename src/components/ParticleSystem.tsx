  ...
  useEffect(() => {
    ...
    if (offCtx) {
      ...
      if (particleStyle === 'sword_qi') {
        // Draw an elongated diamond/shard
        const length = maxSize;
        const width = maxSize / 3;
        offCtx.beginPath();
        offCtx.moveTo(center, center - length); // Top tip
        offCtx.lineTo(center + width, center); // Right point
        offCtx.lineTo(center, center + length); // Bottom tip
        offCtx.lineTo(center - width, center); // Left point
        offCtx.closePath();
        offCtx.fill();
        offCtx.shadowBlur = 0;
        offCtx.fill();
        ...
        offCtx.beginPath();
        offCtx.ellipse(center, center, maxSize, maxSize / 2, 0, 0, Math.PI * 2);
        offCtx.fill();
        offCtx.shadowBlur = 0;
        offCtx.fill();
      } else {
    ...
    const render = (time: number) => {
      ...
      particles.forEach(p => {
        ...
        if (particleStyle === 'lotus_blossom' || particleStyle === 'sword_qi') {
          ctx.save();
          ctx.translate(drawX + drawSize / 2, drawY + drawSize / 2);
          ctx.rotate(p.initialRotation + p.rotationSpeed * (elapsed / 1000));
