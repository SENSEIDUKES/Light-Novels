...
export const ParticleSystem: React.FC<ParticleSystemProps> = React.memo(({ 
  ...
  const particles = useMemo(() => {
    return Array.from({ length: count }).map((_, i) => {
      ...
      const rotationSpeed = (Math.random() - 0.5) * 0.05; // speed of rotation
  ...
  useEffect(() => {
    ...
    if (offCtx) {
      if (particleStyle === 'sword_qi') {
        offCtx.beginPath();
        // Slanted sharp shard (elongated diamond/needle)
        offCtx.moveTo(center, center - maxSize * 1.8);
        offCtx.lineTo(center + maxSize * 0.4, center);
        offCtx.lineTo(center, center + maxSize * 1.8);
        offCtx.lineTo(center - maxSize * 0.4, center);
        offCtx.closePath();
        ...
        offCtx.beginPath();
        // Elegant curved lotus petal shape
        offCtx.moveTo(center, center - maxSize * 1.5);
        offCtx.quadraticCurveTo(center + maxSize * 1.2, center, center, center + maxSize * 1.5);
        offCtx.quadraticCurveTo(center - maxSize * 1.2, center, center, center - maxSize * 1.5);
        offCtx.closePath();
        offCtx.fillStyle = resolvedColor;
        offCtx.shadowBlur = blur;
    ...
    const render = (time: number) => {
      ...
      particles.forEach(p => {
        ...
        if (particleStyle === 'sword_qi') {
          ...
          const angle = Math.PI / 6 + Math.sin(time / 1000 + p.id) * 0.1;
          ctx.rotate(angle);
          ...
          ctx.save();
          ctx.translate(currentX, currentY);
          const angle = p.baseAngle + (time / 1000) * p.rotationSpeed * 5;
          ctx.rotate(angle);
          ctx.drawImage(offscreen, -drawSize / 2, -drawSize / 2, drawSize, drawSize);
          ctx.restore();
        } else {
