...
import type { PersistProfilePictureInput } from './profilePicturePersistence';
...
import {
  ...
  retryPendingProfilePictures,
} from './profilePicturePersistence';
...
function makeInput(
  overrides: Partial<PersistProfilePictureInput> = {},
): PersistProfilePictureInput {
  return {
    userId: 'user-123',
...
describe('cultivator portrait persistence', () => {
  ...
  it('stores the generated source in R2 before selecting the PostgreSQL portrait', async () => {
    const portrait = await persistProfilePicture(makeInput());
  ...
  it('rejects an unauthenticated or cross-account request before uploading', async () => {
    ...
    await expect(persistProfilePicture(makeInput())).rejects.toThrow('does not own');
  ...
  it('returns the durable R2 asset in a deferred error when profile selection fails', async () => {
    ...
    let caught: unknown;
    try {
      await persistProfilePicture(makeInput());
    } catch (error) {
      caught = error;
    }

    expect(caught).toBeInstanceOf(ProfilePictureCommitDeferredError);
    expect((caught as ProfilePictureCommitDeferredError).portrait).toMatchObject({
      id: ASSET_ID,
  ...
  it('surfaces permanent selection rejection instead of claiming recovery is pending', async () => {
    ...
    await expect(persistProfilePicture(makeInput())).rejects.toMatchObject({
  ...
  it('asks the server to recover incomplete portrait selections without local media state', async () => {
    await retryPendingProfilePictures('user-123');
    ...
    await retryPendingProfilePictures('another-user');
  ...
  it('returns the recovered count and surfaces a missing recovery route', async () => {
    ...
    await expect(retryPendingProfilePictures('user-123')).resolves.toBe(1);
    ...
    await expect(retryPendingProfilePictures('user-123'))
